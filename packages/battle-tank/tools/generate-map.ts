...
interface GeneratedMap {
  ...
  // Optional background image path (set by extract mode)
...
program
  ...
  .option('--map-size <size>', 'Map grid size (WxH, e.g., "20x20"; default: 20x20)')
...
/**
 * Parse and validate --map-size flag (WxH format).
 * Returns { rows, cols } or defaults to config.mapSizing.default.
 */
function parseMapSize(sizeStr?: string): { rows: number; cols: number } {
  if (!sizeStr) {
    return MAP_GEN_CONFIG.mapSizing.default;
  }

  const match = sizeStr.match(/^(\d+)x(\d+)$/i);
  if (!match) {
    console.error(`Error: Invalid --map-size format '${sizeStr}'. Use format: WxH (e.g., "20x20")`);
    process.exit(1);
  }

  const rows = parseInt(match[1], 10);
  const cols = parseInt(match[2], 10);
  const { minSize, maxSize, enforceSquare } = MAP_GEN_CONFIG.mapSizing;

  if (rows < minSize || rows > maxSize || cols < minSize || cols > maxSize) {
    console.error(`Error: Map size must be between ${minSize}×${minSize} and ${maxSize}×${maxSize}. Got ${rows}×${cols}`);
    process.exit(1);
  }

  if (enforceSquare && rows !== cols) {
    console.error(`Error: Enforced square maps (rows must equal cols). Got ${rows}×${cols}`);
    process.exit(1);
  }
...
async function runExtractMode() {
  ...
  const overrideSize = options.mapSize ? parseMapSize(options.mapSize) : undefined;
  ...
  const gridRows = overrideSize?.rows ?? originalJson.rows;
  const gridCols = overrideSize?.cols ?? originalJson.cols;
...
TASK: Generate a ${gridRows}×${gridCols} text grid (space-separated symbols per row, newline-separated rows) that maps visible terrain and objects to the archetype legend below.
...
RULES:
- Output dimensions MUST be exactly ${gridRows} rows × ${gridCols} columns
...
- No explanation, no markdown, no extra text`;
  ...
  const refinedMap: GeneratedMap = {
    ...originalJson,
    mockup: generationMockup,
    ...(overrideSize && { rows: overrideSize.rows, cols: overrideSize.cols }),
  };
  ...
  // Add image path to refined map (store relative path for designer to load)
  const imageName = path.basename(resolvedImagePath);
  refinedMap.backgroundImage = imageName;
  ...
  console.log(`✅ Extracted map saved: ${extractedJsonPath}`);
}
...
async function runGenerateMode() {
  ...
  const mapSize = parseMapSize(options.mapSize);
  ...
  const systemPrompt = buildSystemPrompt(schema, themePreset, selectedLandmarks, mapSize);
  ...
  const generatedMap = parseMapResponse(content.text, themeName, additionalPrompt, timeOfDay, season, mapSize);
  if (selectedLandmarks.length > 0) {
  ...
  const imagePrompt = buildImagePrompt(generatedMap, enrichedMetadata, stats, selectedLandmarks);
  const imagePromptPath = path.join(outputDir, `${outputName}_image_prompt.txt`);
  await fs.writeFile(imagePromptPath, imagePrompt);

  console.log(`✅ Map saved: ${jsonPath}`);
...
function buildSystemPrompt(schema: any, themePreset: ThemePreset, landmarks: Landmark[] = [], mapSize?: { rows: number; cols: number }): string {
  ...
  const { minSize, maxSize } = MAP_GEN_CONFIG.mapSizing;
  const gridSize = mapSize || MAP_GEN_CONFIG.mapSizing.default;
...
- HAZARD (Z, U): Avoidance zones.
Note: Battlefield wreckage (tank hulls, helicopter debris) is rendered in the background image only, not as grid objects. Match objects to theme and role. Do NOT confuse terrains with objects (e.g., rocky_outcrop is ground terrain, not an object).
...
1. **Grid Size**: EXACTLY ${gridSize.rows} rows × ${gridSize.cols} columns (MUST be square).
2. **Connectivity**: All spawns connected via walkable paths. Min distance between spawns: ${minSpawnDistance} cells.
3. **Balance**: Follow category targets; max ${maxTerrainVariety} unique terrains.
4. **Cover**: ~${(targetCoverDensity * 100).toFixed(0)}% walkable cells near cover.
5. **Sight & Flow**: Mix exposed lanes with sheltered paths; multiple approaches.
6. **Stats Targets**:
...
**OUTPUT REQUIREMENTS** (strictly follow; output ONLY valid JSON, no extra text/reasoning/markdown):
{
  ...
  "rows": number (MUST be square: rows === cols; must be between ${MAP_GEN_CONFIG.mapSizing.minSize} and ${MAP_GEN_CONFIG.mapSizing.maxSize}),
  "cols": number (MUST be square: rows === cols; must be between ${MAP_GEN_CONFIG.mapSizing.minSize} and ${MAP_GEN_CONFIG.mapSizing.maxSize}),
...
**EXAMPLE** (8x8 square desert; symbols: . = loose_sand, P = hardpan, G = gravel, W = water, C = shipping_container, R = boulder_formation, 1/2 = spawns):
". . . . . . . .\n. . G G . . . .\n. 2 . . . . C .\n. . P P W W . .\n. . P P W W . .\n. . . . C . R .\n. . . . R R . .\n. 1 . . . . . 2"

For urban maps, the default would be different: A = asphalt, u = urban_pavement, r = dirt_road, etc.`;
}

function parseMapResponse(text: string, themeName: string, additionalPrompt: string, timeOfDay: TimeOfDay, season: Season, forcedSize?: { rows: number; cols: number }): GeneratedMap {
  ...
  // Use forced size (from --map-size) or parse from LLM response with validation
  let rows: number, cols: number;
  if (forcedSize) {
    rows = forcedSize.rows;
    cols = forcedSize.cols;
  } else {
  ...
  const { minSize, maxSize, enforceSquare } = MAP_GEN_CONFIG.mapSizing;
  if (rows < minSize || cols < minSize || rows > maxSize || cols > maxSize) {
    throw new Error(`Map dimensions must be between ${minSize} and ${maxSize}. Got ${rows}×${cols}`);
  }

  if (enforceSquare && rows !== cols) {
    const size = Math.max(rows, cols);
    console.warn(`⚠️  Non-square map detected (${rows}×${cols}). Auto-correcting to ${size}×${size}.`);
    rows = size;
    cols = size;
  }
...
/**
 * Build image prompt for external generation service.
 * Describes map layout, terrain, structures, and environment.
 */
function buildImagePrompt(generatedMap: GeneratedMap, metadata: EnrichedMetadata, stats: MapStats, landmarks: Landmark[] = []): string {
  ...
  const allTerrains = getAllTerrains();
  const terrainByName = new Map(allTerrains.map(t => [t.name, t]));
...
TERRAIN & ENVIRONMENT:
The ground features ${terrainList.join(', ')}.
${timePreset.lighting} Colors shift toward ${timePreset.palette}. ${seasonPreset.atmosphere} ${seasonPreset.surfaceEffects}

BATTLEFIELD WRECKAGE:
Integrate scattered tank hulls and helicopter debris throughout the scene as environmental details — burned-out vehicles, twisted fuselage sections, scattered metal fragments. These should appear naturally baked into the terrain and surrounding areas as visual storytelling of ongoing combat, not as distinct grid-level objects.

DESIGN INTENT:
${metadata.designIntention}

TONE: Military reconnaissance photography. Render with natural texture detail and organic variation.
